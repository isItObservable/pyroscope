...
fn init_tracer() -> Result<sdktrace::Tracer, TraceError> {
    ...
    let agent =  PyroscopeAgent::builder(env::var("PYROSCOPE_URL"), "shippingService")
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pyroscope_server = env::var("PYROSCOPE_URL");
    let pprof_config = PprofConfig::new().sample_rate(100);
    let pprof_backend = Pprof::new(pprof_config);

    // Configure Pyroscope Agent
    let agent =PyroscopeAgent::builder(pyroscope_server, "shippingservice")
   .backend(pprof_backend)
   .build()?;
